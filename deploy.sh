...
# 檢查 npm
echo -e "${BLUE}[2/7]${NC} 檢查 npm..."
if ! command -v npm &> /dev/null; then
    echo -e "${RED}❌ 錯誤: 未安裝 npm${NC}"
    exit 1
fi
echo -e "${GREEN}✅ npm $(npm --version) 已安裝${NC}"
echo ""

# 安裝依賴
echo -e "${BLUE}[3/7]${NC} 安裝專案依賴..."
...
# 檢查 Wrangler
echo -e "${BLUE}[4/7]${NC} 檢查 Wrangler CLI..."
...
# 檢查登入狀態
echo -e "${BLUE}[5/7]${NC} 檢查 Cloudflare 登入狀態..."
...
# 部署
echo -e "${BLUE}[7/7]${NC} 部署到 Cloudflare Workers..."
echo ""
echo "準備部署..."
